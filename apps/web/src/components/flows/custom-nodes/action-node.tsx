...
import type { ActionEnrollNodeData } from "@/lib/flows/types";
import { cn } from "@/lib/utils";

export function ActionNode({ data: rawData, selected }: NodeProps) {
  const data = rawData as ActionEnrollNodeData;
  ...
  return (
    <div className={cn("flow-node w-[260px]", selected && "flow-node-selected")}> 
      ...
      <div className="mt-3 flex items-center justify-between gap-2">
        <Badge className={cn("text-[11px]", NODE_CHIP_CLASS["action-enroll"])}>{data.chip}</Badge>
        <p className="text-xs text-zinc-400">{data.sequence}</p>
      </div>

      <Handle
        type="target"
        position={Position.Left}
        className="!h-3 !w-3 !border-2 !border-zinc-950 !bg-cyan-300"
      />
      <Handle
        type="source"
